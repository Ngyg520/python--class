...
class DBManager(object):
        ...
        def get_total_count(self,stu_obj):
            select_sql="select count(*) from %s"%(stu_obj.table_name)#查询总量
            res=self.cursor.execute(select_sql)#执行上一步赋值给res,res为元组的形式
            count=res.fetchone()[0]
            return count

        #count(*)在这里代表对行进行计数，
        #select count(*)from table；这条指令就是会计算出你这个table表里面所有的行数；你也可以这样查询，select
        #select count(*)from table where条件；这样显示的就是对于满足你这个条件的所有行数的计数。

        #定义一个能否查询到ID对应数据的函数
        def get_id_true_or_false(self,stu_obj,number):#number为输入的学生编号
